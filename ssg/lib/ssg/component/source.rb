...
module SSG
  class Component
    class Source
      ...
      def static_files
        @static_files ||= child_files.select(&:static?).index_by do |file|
          file.abs_path.relative_path_from(abs_path.sub_ext("")).to_s
        end
      end

      # @return [Hash<String, SSG::File::TemplateFile>]
      def template_files
        @template_files ||= (sibling_files + child_files).select(&:template?).index_by do |file|
          file.abs_path.basename.to_s
        end
      end
